classdef eventProperties
   ...
   methods
       ...
       function obj = getEventProperties(obj)
           ...
           [event, surounding, EvProp] = Core.eventProperties.geteventPropertieslvl2(startCell, endCell, dataMat, dataEnvMat, sampFreq,  secRad);
       ...
       function obj = getSummary(obj)
          % hfoEve       = obj.Events;
           ...
           % SNR
           obj.Summary.SNR.meanChan        = cellfun(@mean, EventProp.SNR);
           obj.Summary.SNR.meanEnvChan     = cellfun(@mean, EventProp.SNR);
           obj.Summary.SNR.passCount       = cellfun(@sum,  EventProp.mask.SNRpass);
           
           % Amplitude
           obj.Summary.MaxAbsAmpl.event.meanChan    = cellfun(@mean, EventProp.EventMaxAbsAmpl);
           obj.Summary.MaxAbsAmpl.Sur.meanChan    = cellfun(@mean, EventProp.SurMaxAbsAmpl);
           
           obj.Summary.MaxAbsAmpl.passCount   = cellfun(@sum, EventProp.mask.AbsAmplpass);
           obj.Summary.MaxAbsAmpl.passCount   = cellfun(@sum, EventProp.mask.RelAmplpass);
   ...
   methods(Static)
       ...
       function [event, surounding, EventProps] = geteventPropertieslvl2(startCell, endCell, dataMat, dataEnvMat, sampFreq, secRadius) 
...
%            EventProps.EventEnvMaxAbsAmpl = cell(1,nbChan);
           EventProps.SurMaxAbsAmpl    = cell(1,nbChan);
%            EventProps.SurEnvMaxAbsAmpl = cell(1,nbChan);
           %%
           for iChan = 1:nbChan % iterate over channels.
               ...
               %%
               for iEvent = 1:nbEvents % iterae over events.
                   % Event
                   ...
                   SuroundingsIndeces = ...
                      Core.eventProperties.getEventSurrounding(endCell,startCell,iChan,iEvent,nbSamp,Radi,true);
                   SuroundingsIndecesWithOtherHFO = ...
                      Core.eventProperties.getEventSurrounding(endCell,startCell,iChan,iEvent,nbSamp,Radi,false);
...
%                    SuroundingEnvDataWithOtherHFO = dataEnvMat(SuroundingsIndecesWithOtherHFO, iChan);
...
%                    EventProps.eventMeanEnv{iChan}(iEvent)     = mean(eventEnvData);
%                    EventProps.EventEnvMaxAbsAmpl{iChan}(iEvent) = max(abs(eventData));
...
%                    EventProps.SurMeanEnv{iChan}(iEvent)       = mean(SuroundingEnvDataWithOtherHFO); 
%                    EventProps.SurMaxAbsEnvAmpl{iChan}(iEvent) = max(abs(SuroundingEnvDataWithOtherHFO));                   
%                    EventProps.SurRMS{iChan}(iEvent)         = rms(SuroundingData);

                   %OverAll 
                   EventProps.SNR{iChan}(iEvent)                =(rms(eventData)/rms(SuroundingData))^2;
%                    EventProps.envSNR{iChan}(iEvent)             =(rms(eventEnvData)/rms(SuroundingEnvData))^2;
               end
           end
       end
       % utility level 1
       function SuroundingsIndeces = getEventSurrounding(endCell,startCell,iChan,iEvent,nbSamp,Radi,isClean)
           ...
           [preEventEnd, postEventStr] = ...
               Core.eventProperties.getFirstAndLastEventCase(eventStr, eventEnd, nbSamp, Radi);

           maxBefor = preEventEnd;
           minAfter = postEventStr;
           SuroundingsIndeces = [maxBefor:(eventStr-1), (1+eventEnd):minAfter];
           if isClean
           surEventMask = Core.eventProperties.getSuroundingEventsMask(endCell,startCell,iChan,SuroundingsIndeces);
